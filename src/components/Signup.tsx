...
export default function Signup() {
  ...
  const submitHandler = async (e: React.FormEvent<HTMLFormElement>) => {
    try {
      ...
      console.log("result form frontend signup",res);
      if (res) {
        setSignUP({
          firstName: "",
          lastName: "",
      ...
      console.log("check data from step by step:",err?.response?.data?.message);
      ...
      <div className="flex flex-col w-1/3 h-auto border-2 bg-gradient-to-b from-red-500 to-blue-500 opacity-70 ">
        ...
        <form onSubmit={submitHandler}>
          ...
          <div className="flex justify-center items-center">
            <button className="border text-2xl m-2 p-2 rounded-2xl active:bg-red-500">
              {" "}
              Sign UP{" "}
            </button>
          </div>
        ...
        <div className="flex m-3 mx-3 ">
          <p> Already Account!</p>
          <Link to="/login">
            <button className="mx-2 font-bold active:text-rose-500">
              {" "}
              Log In Now{" "}
            </button>
          </Link>
